...
import { TeacherAdvanceSchema, TeacherAdvanceSchemaType } from "@workspace/utils/schema"

import { useGetAdvances } from "../../hooks/use-get-advances";

interface EditTeacherAdvanceFormViewProps {
    id: string
}

export const EditTeacherAdvanceFormView = ({ id }: EditTeacherAdvanceFormViewProps) => {
    ...
    const { data } = useSuspenseQuery(trpc.teacherAdvance.getOne.queryOptions(id))
    ...
    const { mutate: updateAdvance, isPending } = useMutation(
        trpc.teacherAdvance.updateOne.mutationOptions({
            ...
            onSuccess: async (data) => {
                ...
                await Promise.all([
                    queryClient.invalidateQueries(trpc.teacherAdvance.getMany.queryOptions({ ...filters })),
                    queryClient.invalidateQueries(trpc.teacherAdvance.getOne.queryOptions(id))
                ]);
