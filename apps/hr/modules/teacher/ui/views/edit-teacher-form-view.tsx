...
import { z } from "zod";
import { DollarSign, Edit, Loader2, MapPin, PhoneCall, School, User, Clock } from "lucide-react";
...
import { TeacherSchema, TeacherSchemaType } from "@workspace/utils/schema";
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from "@workspace/ui/components/form";
import { cn } from "@workspace/ui/lib/utils";
import { Separator } from "@workspace/ui/components/separator";
import { Badge } from "@workspace/ui/components/badge";
import { DAYS, GENDER, LEVEL, LEVELS, NATIONALITY, RELIGION, STATUS, TEACHER_STATUS, timeSlots } from "@workspace/utils/constant";
...
const STEPS = [
    ...
    },
    {
        id: 6,
        name: "Rate & Details",
        fields: ["classRate", "teacherId", "status"] as const,
        Icon: DollarSign,
    },
] as const;
...
const SELECT_OPTIONS = {
    ...
    levels: Object.values(LEVEL).map((v) => ({ label: v, value: v })),
    days: Object.values(DAYS).map((v) => ({ label: v, value: v })),
    times: Object.values(timeSlots).map((v) => ({ label: v, value: v })),
    status: Object.values(STATUS).map((v) => ({ label: v, value: v })),
};
...
const PersonalInfoStep = ({ form, trigger, isPending }: StepProps) => (
    <div className="grid md:grid-cols-2 gap-6 items-start">
        <FormInput form={form} name="name" label="Full Name" type="text" trigger={trigger} disabled={isPending} />
        <FormInput form={form} name="fName" label="Father's Name" type="text" trigger={trigger} disabled={isPending} />
        <FormInput form={form} name="mName" label="Mother's Name" type="text" trigger={trigger} disabled={isPending} />
...
const AddressStep = ({ form, trigger, isPending }: StepProps) => (
    <div className="flex flex-col gap-y-4">
        ...
        <div className="grid md:grid-cols-2 gap-6 items-start">
            ...
            <FormInput form={form} name="presentMoholla" label="Moholla/Area" type="text" trigger={trigger} disabled={isPending} />
            <FormInput form={form} name="presentPost" label="Post Office" type="text" trigger={trigger} disabled={isPending} />
            <FormInput form={form} name="presentThana" label="Thana/Upazila" type="text" trigger={trigger} disabled={isPending} />
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 items-start">
            <FormInput form={form} name="permanentVillage" label="Village/Area" type="text" trigger={trigger} disabled={isPending} />
            <FormInput form={form} name="permanentPost" label="Post Office" type="text" trigger={trigger} disabled={isPending} />
            <FormInput form={form} name="permanentThana" label="Thana/Upazila" type="text" trigger={trigger} disabled={isPending} />
...
const ContactStep = ({ form, trigger, isPending }: StepProps) => (
    <div className="grid md:grid-cols-2 gap-6 items-start">
        <FormInput form={form} name="phone" label="Primary Phone" type="text" trigger={trigger} disabled={isPending} />
        <FormInput form={form} name="altPhone" label="Alternative Phone" type="text" trigger={trigger} disabled={isPending} />
    </div>
);
...
const RateDetailsStep = ({ form, trigger, isPending, editStates, toggleEdit, isUpdating }: RateDetailsStepProps) => (
    <div className="grid md:grid-cols-2 gap-6 items-start">
        <EditableField
            form={form}
            name="classRate"
        ...
        <FormSelect
            ...
            name="status"
            label="Status"
            options={Object.values(TEACHER_STATUS).map((status) => ({ label: status, value: status }))}
...
export const EditTeacherFormView = ({ id }: EditTeacherFormProps) => {
    ...
    const { data: teacher } = useSuspenseQuery(trpc.teacher.getOne.queryOptions(id));

    const { mutate: updateTeacher, isPending } = useMutation(trpc.teacher.updateOne.mutationOptions({
        onError: (err) => {
    ...
    useEffect(() => {
        if (teacher) {
            const teacherData = {
                ...teacher,
                dob: teacher.dob ? new Date(teacher.dob).toISOString() : new Date().toISOString(),
                level: teacher.level || [],
    ...
    const onSubmit = useCallback((data: TeacherSchemaType) => {
    ...
    const renderStepContent = useMemo(() => {
        ...
        switch (currentStep) {
            ...
            case 6:
                return <RateDetailsStep {...stepProps} editStates={editStates} toggleEdit={toggleEdit} isUpdating={isPending} />;
            default:
                return null;
        }
